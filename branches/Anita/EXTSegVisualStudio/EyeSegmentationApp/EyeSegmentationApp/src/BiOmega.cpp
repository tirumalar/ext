...
//22 Jun2011		m_pEyeSegmentInterface->EnableEyelidSegmentation(true);
		...
		//m_pEyeSegmentInterface->SetEyeLocationSearchArea(180, 120, 220, 180);
		m_pEyeSegmentInterface->SetEyeLocationSearchArea(240, 180, 160, 120); //Default
		//m_pEyeSegmentInterface->SetPupilAngleSearchRange(minAngle, maxAngle);
...
std::pair<int, float> BiOmega::MatchIris(unsigned char *imageBuffer, int w, int h, int stride, char *irisCodeDatabase, int numberOfEyes,IrisPupilCircles *pCircles,CvPoint2D32f *pRefCentre,CvPoint2D32f *pVar,int specrad)
{
	...
	if(pRefCentre)
	{
	    ...
	    // IrisSelectServer::ComputeSpecularityMetrics(pyr[1],RefCentre,Var,specrad); // Todo Anita
