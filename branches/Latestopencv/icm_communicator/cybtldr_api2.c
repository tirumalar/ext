...
int CyBtldr_RunAction(CyBtldr_Action action, const char* file, CyBtldr_CommunicationsData* comm, CyBtldr_ProgressUpdate* update)
{
    ...
    {
        if(debugFlag){ printf("Progress: opening bootloadable file %s : %d \n",file,err); };
        err = CyBtldr_GetFileSize(file,&fileSize); 
        err = CyBtldr_ReadLine(&lineLen, line);
        if (CYRET_SUCCESS == err)
            err = CyBtldr_ParseHeader(lineLen, line, &siliconId, &siliconRev, &chksumtype);

	if(debugFlag){ printf("Progress: parsing header %d \n",err); };	
        if (CYRET_SUCCESS == err)
        {
	...
	if(debugFlag){ printf("Progress: starting bootload operation :%d\n",err); };   
        app = strrchr(file, '_');
        appId = (app != NULL && '.' == app[2])? (CyBtldr_FromHex(app[1]) - 1): INVALID_APP;
	    ...
	    { /* Single app - restore previous CYRET_SUCCESS */
	      err = CYRET_SUCCESS;
            ...
            if(debugFlag){ printf("Progress: getting application status: %d \n",err); };  
        }
        ...
        if (CYRET_SUCCESS == err)
        {
                ...
                if (CYRET_SUCCESS == err)
                {
                    switch (action)
                    {
                        case ERASE:
                            ...
                            break;
                        case PROGRAM:
			    if(debugFlag){ printf("Progress: currently programming row : %d \n",rowNum); };	
            ...
            if (CYRET_SUCCESS == err)
            {
               ...
               if(debugFlag){ printf("Progress: verifying program: %d \n",err); };  
            }
