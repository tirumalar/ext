#ifndef MAM_TEST_REPORTER
#define MAM_TEST_REPORTER
...
class mamigo_test_reporter: public tut::reporter{
	ofstream xRes;
	apr_time_t lastTime;
	int ignored_count;
...
public:
	mamigo_test_reporter():ignored_count(0),xRes ("testResults.xml",ios_base::out|ios_base::trunc)
	{
		xRes<<"<?xml version=\"1.0\" encoding=\"UTF-8\" ?>"<<endl;
		xRes<<"<tests>"<<endl;
	}
	~mamigo_test_reporter(){
		xRes<<"</tests>"<<endl;
		xRes.close();
	}
	...
	void test_completed(const test_result& tr)
	{
		...
		{
			xRes<<"<"<<result<<">"<<endl;
			string fMsg(tr.message);
			xRes << xml_encode(fMsg);
			xRes<<"</"<<result<<">"<<endl;
		}
		xRes<<"</testcase>"<<endl;
		lastTime=apr_time_now();
...
static void run_tests_ignoring(ignoredTests& ignored, mamigo_test_reporter& reporter){
