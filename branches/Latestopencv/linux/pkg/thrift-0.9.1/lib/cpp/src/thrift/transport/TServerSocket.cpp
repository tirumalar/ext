...
// ATH-1537
// ****
void TServerSocket::setKeepAlive(bool keepAlive) {
  keepAlive_ = keepAlive;
}

void TServerSocket::setKeepAliveTime(int keepAliveTime) {
  keepAliveTime_ = keepAliveTime;
}

void TServerSocket::setKeepAliveIntvl(int keepAliveIntvl) {
  keepAliveIntvl_ = keepAliveIntvl;
}

void TServerSocket::setKeepAliveProbes(int keepAliveProbes) {
  keepAliveProbes_ = keepAliveProbes;
}
// ****
...
void TServerSocket::listen() {
  ...
  if (! path_.empty()) {
    ...
    printf("serverSocket_ -> %d\n",serverSocket_);
  } else {
    serverSocket_ = socket(res->ai_family, res->ai_socktype, res->ai_protocol);
    printf("serverSocket_ -> %d %d \n",serverSocket_,res->ai_socktype);
  }
  //Mamigo
...
shared_ptr<TTransport> TServerSocket::acceptImpl() {
  ...
  // ATH-1537
  // ****
  int clientSocketFd = THRIFT_INVALID_SOCKET;
  if (keepAlive_)
  {
	  clientSocketFd = client->getSocketFD();
	  if (clientSocketFd == THRIFT_INVALID_SOCKET)
	  {
		  int errno_copy = THRIFT_GET_SOCKET_ERROR;
		  GlobalOutput.perror("TServerSocket::acceptImpl() client->getSocketFD() returned THRIFT_INVALID_SOCKET", errno_copy);
		  throw TTransportException(TTransportException::UNKNOWN, "client->getSocketFD()", errno_copy);
	  }

	  int v = 1;
      if (setsockopt(clientSocketFd, SOL_SOCKET, SO_KEEPALIVE, cast_sockopt(&v), sizeof(v)) < 0) {
    	  int errno_copy = THRIFT_GET_SOCKET_ERROR;
    	  GlobalOutput.perror("TServerSocket::acceptImpl() setsockopt() SO_KEEPALIVE", errno_copy);
    	  throw TTransportException(TTransportException::UNKNOWN, "setsockopt() SO_KEEPALIVE", errno_copy);
	  }

      if (setsockopt(clientSocketFd, SOL_TCP, TCP_KEEPCNT, cast_sockopt(&keepAliveProbes_), sizeof(keepAliveProbes_)) < 0) {
    	  int errno_copy = THRIFT_GET_SOCKET_ERROR;
    	  GlobalOutput.perror("TServerSocket::acceptImpl() setsockopt() TCP_KEEPCNT", errno_copy);
    	  throw TTransportException(TTransportException::UNKNOWN, "setsockopt() TCP_KEEPCNT", errno_copy);
      }

      if (setsockopt(clientSocketFd, SOL_TCP, TCP_KEEPIDLE, cast_sockopt(&keepAliveTime_), sizeof(keepAliveTime_)) < 0) {
    	  int errno_copy = THRIFT_GET_SOCKET_ERROR;
    	  GlobalOutput.perror("TServerSocket::acceptImpl() setsockopt() TCP_KEEPIDLE", errno_copy);
    	  throw TTransportException(TTransportException::UNKNOWN, "setsockopt() TCP_KEEPIDLE", errno_copy);
      }

      if (setsockopt(clientSocketFd, SOL_TCP, TCP_KEEPINTVL, cast_sockopt(&keepAliveIntvl_), sizeof(keepAliveIntvl_)) < 0) {
    	  int errno_copy = THRIFT_GET_SOCKET_ERROR;
    	  GlobalOutput.perror("TServerSocket::acceptImpl() setsockopt() TCP_KEEPINTVL", errno_copy);
    	  throw TTransportException(TTransportException::UNKNOWN, "setsockopt() TCP_KEEPINTVL", errno_copy);
      }
  }
  // ****
