...
int EyeLockMain::CheckTamperSensor() // internal API
{
	...
	if(m_Master){
		...
		status = I2CBusNanoAPI::instance().Write( 4, 3 );	// COMMAND=4, TAMPER=3
		if(status) {
			EyelockLog(logger, ERROR, "Failed to write register COMMAND 4, value 3 for tamper sensor " );
			return 0;
		}
		usleep(100);
		status = I2CBusNanoAPI::instance().Read( 5, &tamper );	// SENSOR
		if(status) {
			EyelockLog(logger, ERROR, "Failed to read tamper register" );
			return 0;
		}
		//EyelockLog(logger, DEBUG, "tamper sensor: tamper=%d", tamper );
	}
#endif
	if (access("OIMtamper", F_OK ) != -1)  // tamper file will generated by FaceTracker
	{
		if (m_Debug)
