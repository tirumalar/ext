...
volatile uint8   I2C_1_slOverFlowCount; /* Slave Transmit Overflow counter */
    ...
    uint32 (*I2C_1_customAddressHandler)(void) = NULL;

    /*******************************************************************************
    * Function Name: I2C_1_SetI2cAddressCustomInterruptHandler
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
    void I2C_1_SetI2cAddressCustomInterruptHandler(uint32 (*func) (void))
    {
        ...
        I2C_1_customAddressHandler = func;
    }
...
*   - I2C_1_I2C_SSTAT_RD_CMPLT - Slave read transfer complete. 
...
*   - I2C_1_I2C_SSTAT_RD_BUSY - Slave read transfer is in progress. 
*     Set when master addresses slave with a read, cleared when 
*    I2C_1_I2C_SSTAT_RD_CMPLT is set.
*   - I2C_1_I2C_SSTAT_RD_OVFL - Master attempted to read more bytes
*     than are in buffer. Slave continually returns 0xFF byte in this case.
*   - I2C_1_I2C_SSTAT_RD_ERR - Slave captured error on the bus during 
*     a read transfer. The sources of error are: misplaced Start or Stop 
*     condition or lost arbitration while slave drives SDA.
*   - I2C_1_I2C_SSTAT_WR_CMPLT - Slave write transfer complete. 
...
*     successfully.
*   - I2C_1_I2C_SSTAT_WR_BUSY - Slave write transfer is in progress. 
*     Set when the master addresses the slave with a write, cleared when 
*    I2C_1_I2C_SSTAT_WR_CMPLT is set.
*   - I2C_1_I2C_SSTAT_WR_OVFL - Master attempted to write past end of 
*     buffer. Further bytes are ignored.
*   - I2C_1_I2C_SSTAT_WR_ERR - Slave captured error on the bus during
...
*     I2C_1_I2C_SSTAT_WR_ERR is set. It is recommended to discard write 
...
*
* \sideeffect
*  This function does not clear I2C_1_I2C_SSTAT_RD_BUSY.
*
* \globalvars
*  I2C_1_slStatus  - used to store current status of I2C slave.
*
...
*
* \sideeffect
*  This function does not clear I2C_1_I2C_SSTAT_WR_BUSY.*
*
* \globalvars
*  I2C_1_slStatus  - used to store current status of I2C slave.
*
...
void I2C_1_I2CSlaveSetAddressMask(uint32 addressMask)
{
    ...
    matchReg &= ((uint32) ~I2C_1_RX_MATCH_MASK_MASK);
...
*  I2C_1_SlaveGetReadBufSize() function.
*
...
*  resets the transfer count returned with the 
*  I2C_1_I2CSlaveGetWriteBufSize() function.
*
...
*  I2C_1_I2CSlaveInitReadBuf() or 
*  I2C_1_I2CSlaveClearReadBuf() function was executed.
...
*
* \sideeffect
*  The returned value is not valid if I2C_1_I2C_SSTAT_RD_ERR was 
...
*  I2C_1_I2CSlaveInitWriteBuf() or 
*  I2C_1_I2CSlaveClearWriteBuf() function was executed.
...
*
* \sideeffect
*  The returned value is not valid if I2C_1_I2C_SSTAT_WR_ERR was 
*  captured by the slave.
*
* \globalvars
*  I2C_1_slWrBufIndex - used to store current index within slave
