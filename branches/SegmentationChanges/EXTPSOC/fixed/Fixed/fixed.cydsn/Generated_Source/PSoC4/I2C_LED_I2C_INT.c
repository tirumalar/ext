...
/*******************************************************************************
* Function Name: I2C_LED_I2C_ISR
****************************************************************************//**
*
*  Handles the Interrupt Service Routine for the SCB I2C mode.
*
*******************************************************************************/
CY_ISR(I2C_LED_I2C_ISR)
{
    uint32 diffCount;
    uint32 endTransfer;

#ifdef I2C_LED_I2C_ISR_ENTRY_CALLBACK
    I2C_LED_I2C_ISR_EntryCallback();
#endif /* I2C_LED_I2C_ISR_ENTRY_CALLBACK */

#if (I2C_LED_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
    uint32 response;

    response = I2C_LED_I2C_ACK_ADDR;
    ...
    if(NULL != I2C_LED_customIntrHandler)
    {
        I2C_LED_customIntrHandler();
    }

    if(I2C_LED_CHECK_INTR_I2C_EC_MASKED(I2C_LED_INTR_I2C_EC_WAKE_UP))
    {
        /* Mask-off after wakeup */
        I2C_LED_SetI2CExtClkInterruptMode(I2C_LED_NO_INTR_SOURCES);
    }
    ...
    */
    if(I2C_LED_CHECK_I2C_FSM_MASTER)
    {
        #if(I2C_LED_I2C_MASTER)
        {
            ...
            */
            if(I2C_LED_CHECK_INTR_MASTER_MASKED(I2C_LED_INTR_MASTER_I2C_BUS_ERROR))
            {
                I2C_LED_mstrStatus |= (uint16) (I2C_LED_I2C_MSTAT_ERR_XFER |
                                                         I2C_LED_I2C_MSTAT_ERR_BUS_ERROR);

                endTransfer = I2C_LED_I2C_CMPLT_ANY_TRANSFER;
            }
            ...
            */
            if(I2C_LED_CHECK_INTR_MASTER_MASKED(I2C_LED_INTR_MASTER_I2C_ARB_LOST))
            {
                I2C_LED_mstrStatus |= (uint16) (I2C_LED_I2C_MSTAT_ERR_XFER |
                                                         I2C_LED_I2C_MSTAT_ERR_ARB_LOST);

                endTransfer = I2C_LED_I2C_CMPLT_ANY_TRANSFER;
            }

            #if(I2C_LED_I2C_MULTI_MASTER_SLAVE)
            {
                /* I2C_MASTER_CMD_M_START_ON_IDLE:
                * MultiMaster-Slave does not generate start, because Slave was addressed.
                * Pass control to slave.
                */
                if(I2C_LED_CHECK_I2C_MASTER_CMD(I2C_LED_I2C_MASTER_CMD_M_START_ON_IDLE))
                {
                    I2C_LED_mstrStatus |= (uint16) (I2C_LED_I2C_MSTAT_ERR_XFER |
                                                             I2C_LED_I2C_MSTAT_ERR_ABORT_XFER);
            ...
            */
            if(0u != endTransfer)
            {
                ...
                I2C_LED_mstrStatus |= (uint16) I2C_LED_GET_I2C_MSTAT_CMPLT;

                #if(I2C_LED_I2C_MULTI_MASTER_SLAVE)
                {
                    if(I2C_LED_CHECK_I2C_FSM_ADDR)
                    {
                        ...
                        */
                        if(I2C_LED_CHECK_I2C_MASTER_CMD(I2C_LED_I2C_MASTER_CMD_M_START_ON_IDLE))
                        {
                            I2C_LED_I2C_MASTER_CLEAR_START;

                            endTransfer = I2C_LED_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        ...
                        */
                        else if((!I2C_LED_CHECK_INTR_SLAVE_MASKED(I2C_LED_INTR_SLAVE_I2C_BUS_ERROR))
                               && I2C_LED_CHECK_INTR_MASTER_MASKED(I2C_LED_INTR_MASTER_I2C_ARB_LOST))
                        {
                            endTransfer = I2C_LED_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        else
                        ...
                        {
                            I2C_LED_CLEAR_TX_FIFO; /* Shifter keeps address, clear it */

                            I2C_LED_DISABLE_MASTER_AUTO_DATA_ACK; /* In case of reading disable autoACK */
                            ...
                            I2C_LED_SetRxInterruptMode(I2C_LED_NO_INTR_SOURCES);
                        ...
                        else
                        {
                            ...
                            I2C_LED_state = I2C_LED_I2C_FSM_EXIT_IDLE;
                        }
                    }
                    else
                    {
    ...
    else /* (I2C_LED_CHECK_I2C_FSM_SLAVE) */
    {
        #if(I2C_LED_I2C_SLAVE)
        {
            /* INTR_SLAVE_I2C_BUS_ERROR or I2C_LED_INTR_SLAVE_I2C_ARB_LOST:
            * A Misplaced Start or Stop condition occurred on the bus: set a flag
            * to notify an error condition.
            */
            if(I2C_LED_CHECK_INTR_SLAVE_MASKED(I2C_LED_INTR_SLAVE_I2C_BUS_ERROR |
                                                        I2C_LED_INTR_SLAVE_I2C_ARB_LOST))
            {
                if(I2C_LED_CHECK_I2C_FSM_RD)
                {
                    /* TX direction: master reads from slave */
                    I2C_LED_slStatus &= (uint8) ~I2C_LED_I2C_SSTAT_RD_BUSY;
                    I2C_LED_slStatus |= (uint8) (I2C_LED_I2C_SSTAT_RD_ERR |
                                                          I2C_LED_I2C_SSTAT_RD_CMPLT);
                }
                else
                {
                    /* RX direction: master writes into slave */
                    I2C_LED_slStatus &= (uint8) ~I2C_LED_I2C_SSTAT_WR_BUSY;
                    I2C_LED_slStatus |= (uint8) (I2C_LED_I2C_SSTAT_WR_ERR |
                                                          I2C_LED_I2C_SSTAT_WR_CMPLT);
                }
    ...
    */
    ...
    if(I2C_LED_CHECK_I2C_FSM_MASTER)
    {
        #if(I2C_LED_I2C_MASTER)
        {
            ...
            */
            if(I2C_LED_CHECK_INTR_MASTER_MASKED(I2C_LED_INTR_MASTER_I2C_STOP))
            {
            ...
            {
                if(I2C_LED_CHECK_I2C_FSM_ADDR) /* Address stage */
                {
                    /* INTR_MASTER_I2C_NACK:
                    * The master sent an address but it was NACKed by the slave. Complete transaction.
                    */
                    if(I2C_LED_CHECK_INTR_MASTER_MASKED(I2C_LED_INTR_MASTER_I2C_NACK))
                    {
                        I2C_LED_ClearMasterInterruptSource(I2C_LED_INTR_MASTER_I2C_NACK);

                        I2C_LED_mstrStatus |= (uint16) (I2C_LED_I2C_MSTAT_ERR_XFER |
                                                                 I2C_LED_I2C_MSTAT_ERR_ADDR_NAK);
                    ...
                    */
                    ...
                    {
                        if(I2C_LED_CHECK_I2C_FSM_RD) /* Reading */
                        {
                        ...
                        else /* Writing */
                        {
                            ...
                            if(0u != I2C_LED_mstrWrBufSize)
                            {
                                /* Enable INTR.TX_EMPTY if there is data to transmit */
                                I2C_LED_SetTxInterruptMode(I2C_LED_INTR_TX_EMPTY);
                            }
                        }
                    }
                }

                if(I2C_LED_CHECK_I2C_FSM_DATA) /* Data phase */
                {
                    if(I2C_LED_CHECK_I2C_FSM_RD) /* Reading */
                    {
                        /* INTR_RX_FULL:
                        ...
                        */
                        if(I2C_LED_CHECK_INTR_RX_MASKED(I2C_LED_INTR_RX_FULL))
                        {
                            /* Calculate difference */
                            diffCount =  I2C_LED_mstrRdBufSize -
                                        (I2C_LED_mstrRdBufIndex + I2C_LED_GET_RX_FIFO_ENTRIES);

                            /* Proceed transaction or end it when RX FIFO becomes FULL again */
                            if(diffCount > I2C_LED_I2C_FIFO_SIZE)
                            {
                                diffCount = I2C_LED_I2C_FIFO_SIZE;
                            }
                            else
                            {
                                if(0u == diffCount)
                                {
                                    I2C_LED_DISABLE_MASTER_AUTO_DATA_ACK;
                            ...
                            {
                                I2C_LED_mstrRdBufPtr[I2C_LED_mstrRdBufIndex] = (uint8)
                                                                                        I2C_LED_RX_FIFO_RD_REG;
                                I2C_LED_mstrRdBufIndex++;
                            }
                        }
                        /* INTR_RX_NOT_EMPTY:
                        ...
                        */
                        else if(I2C_LED_CHECK_INTR_RX_MASKED(I2C_LED_INTR_RX_NOT_EMPTY))
                        {
                            /* Put data in component buffer */
                            I2C_LED_mstrRdBufPtr[I2C_LED_mstrRdBufIndex] = (uint8) I2C_LED_RX_FIFO_RD_REG;
                            I2C_LED_mstrRdBufIndex++;

                            if(I2C_LED_mstrRdBufIndex < I2C_LED_mstrRdBufSize)
                            {
                        ...
                        I2C_LED_ClearRxInterruptSource(I2C_LED_INTR_RX_ALL);
                    }
                    else /* Writing */
                    {
                        /* INTR_MASTER_I2C_NACK :
                        ...
                        */
                        if(I2C_LED_CHECK_INTR_MASTER_MASKED(I2C_LED_INTR_MASTER_I2C_NACK))
                        {
                            ...
                            I2C_LED_mstrWrBufIndexTmp -= (I2C_LED_GET_TX_FIFO_ENTRIES +
                                                                   I2C_LED_GET_TX_FIFO_SR_VALID);

                            /* Update number of transferred bytes */
                            I2C_LED_mstrWrBufIndex = I2C_LED_mstrWrBufIndexTmp;

                            I2C_LED_mstrStatus |= (uint16) (I2C_LED_I2C_MSTAT_ERR_XFER |
                                                                     I2C_LED_I2C_MSTAT_ERR_SHORT_XFER);

                            I2C_LED_CLEAR_TX_FIFO;
                        ...
                        */
                        else if(I2C_LED_CHECK_INTR_TX_MASKED(I2C_LED_INTR_TX_EMPTY))
                        {
                            while(I2C_LED_I2C_FIFO_SIZE != I2C_LED_GET_TX_FIFO_ENTRIES)
                            {
                                ...
                                */
                                if(I2C_LED_mstrWrBufIndexTmp < I2C_LED_mstrWrBufSize)
                                {
                                    ...
                                    */
                                    if(I2C_LED_mstrWrBufIndexTmp == (I2C_LED_mstrWrBufSize - 1u))
                                    {
                                    ...
                                    I2C_LED_TX_FIFO_WR_REG = (uint32) I2C_LED_mstrWrBufPtr[I2C_LED_mstrWrBufIndexTmp];
                                    I2C_LED_mstrWrBufIndexTmp++;
                                }
                                else
                                {
                            ...
                            I2C_LED_ClearTxInterruptSource(I2C_LED_INTR_TX_ALL);
                        #else
                            I2C_LED_ClearTxInterruptSource(I2C_LED_INTR_TX_EMPTY);
                        #endif /* (I2C_LED_CY_SCBIP_V0) */
                        }
                        /* INTR_TX_UNDERFLOW:
                        ...
                        */
                        else if(I2C_LED_CHECK_INTR_TX_MASKED(I2C_LED_INTR_TX_UNDERFLOW))
                        {
                            /* Update number of transferred bytes */
                            I2C_LED_mstrWrBufIndex = I2C_LED_mstrWrBufIndexTmp;
                ...
                if(0u != endTransfer) /* Complete transfer */
                {
                    ...
                    if(I2C_LED_CHECK_I2C_MODE_NO_STOP(I2C_LED_mstrControl))
                    {
    ...
    } /* (I2C_LED_CHECK_I2C_FSM_MASTER) */


    /* FSM Slave */
    else if(I2C_LED_CHECK_I2C_FSM_SLAVE)
    {
        #if(I2C_LED_I2C_SLAVE)
        {
            /* INTR_SLAVE_NACK:
            ...
            */
            if(I2C_LED_CHECK_INTR_SLAVE_MASKED(I2C_LED_INTR_SLAVE_I2C_NACK))
            {
                I2C_LED_ClearSlaveInterruptSource(I2C_LED_INTR_SLAVE_I2C_NACK);

                /* All entries that remain in TX FIFO are: FIFO Size + 1 (SHIFTER) */
                diffCount = (I2C_LED_GET_TX_FIFO_ENTRIES + I2C_LED_GET_TX_FIFO_SR_VALID);

                if(I2C_LED_slOverFlowCount > diffCount) /* Overflow */
                {
                    I2C_LED_slStatus |= (uint8) I2C_LED_I2C_SSTAT_RD_OVFL;
                }
                else /* No Overflow */
                {
                    /* Roll-back temporary index */
                    I2C_LED_slRdBufIndexTmp -= (diffCount - I2C_LED_slOverFlowCount);
                }

                /* Update slave of transferred bytes */
                I2C_LED_slRdBufIndex = I2C_LED_slRdBufIndexTmp;

                /* Clean-up TX FIFO */
                I2C_LED_SetTxInterruptMode(I2C_LED_NO_INTR_SOURCES);
                I2C_LED_slOverFlowCount = 0u;
                I2C_LED_CLEAR_TX_FIFO;

                /* Complete master reading */
                I2C_LED_slStatus &= (uint8) ~I2C_LED_I2C_SSTAT_RD_BUSY;
                I2C_LED_slStatus |= (uint8)  I2C_LED_I2C_SSTAT_RD_CMPLT;
                I2C_LED_state     =  I2C_LED_I2C_FSM_IDLE;
            }
            ...
            */
            if(I2C_LED_CHECK_INTR_SLAVE_MASKED(I2C_LED_INTR_SLAVE_I2C_WRITE_STOP))
            {
                I2C_LED_ClearSlaveInterruptSource(I2C_LED_INTR_SLAVE_I2C_WRITE_STOP);
                ...
                * I2C_ADDR_MATCH.
                */
                if (0u != (I2C_LED_I2C_CTRL_REG & I2C_LED_I2C_CTRL_S_READY_DATA_ACK))
                {
                    while(0u != I2C_LED_GET_RX_FIFO_ENTRIES)
                    {
                        #if(I2C_LED_CHECK_I2C_ACCEPT_ADDRESS)
                        {
                            if((1u == I2C_LED_GET_RX_FIFO_ENTRIES) &&
                               (I2C_LED_CHECK_INTR_SLAVE_MASKED(I2C_LED_INTR_SLAVE_I2C_ADDR_MATCH)))
                            {
                        ...
                        /* Put data in component buffer */
                        I2C_LED_slWrBufPtr[I2C_LED_slWrBufIndex] = (uint8) I2C_LED_RX_FIFO_RD_REG;
                        I2C_LED_slWrBufIndex++;
                    }

                    I2C_LED_DISABLE_SLAVE_AUTO_DATA;
                }

                if(I2C_LED_CHECK_INTR_RX(I2C_LED_INTR_RX_OVERFLOW))
                {
                    I2C_LED_slStatus |= (uint8) I2C_LED_I2C_SSTAT_WR_OVFL;
                }
                ...
                I2C_LED_slStatus &= (uint8) ~I2C_LED_I2C_SSTAT_WR_BUSY;
                I2C_LED_slStatus |= (uint8)  I2C_LED_I2C_SSTAT_WR_CMPLT;
                I2C_LED_state     =  I2C_LED_I2C_FSM_IDLE;
            }
            ...
            */
        #if (I2C_LED_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
            if (I2C_LED_CHECK_INTR_SLAVE_MASKED(I2C_LED_INTR_SLAVE_I2C_ADDR_MATCH |
        ...
        #endif /* (I2C_LED_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
            {
                ...
                I2C_LED_ClearI2CExtClkInterruptSource(I2C_LED_INTR_I2C_EC_WAKE_UP);

                #if (I2C_LED_I2C_CUSTOM_ADDRESS_HANDLER)
                {
                    if (NULL != I2C_LED_customAddressHandler)
                    {
                        /* Call custom address handler */
                        response = I2C_LED_customAddressHandler();
                    }
                    else
                    {
                        /* Read address from the RX FIFO. If there is no address underflow triggers but
                        * component does not use that source. */
                        (void) I2C_LED_RX_FIFO_RD_REG;
                        response = I2C_LED_I2C_ACK_ADDR;
                    }
            ...
            #if (I2C_LED_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
                if (response == I2C_LED_I2C_NAK_ADDR)
                {
                #if (!I2C_LED_CY_SCBIP_V0)
                    /* Disable write stop interrupt source as it triggers after address was NACKed. Ticket ID#156094 */
                    I2C_LED_DISABLE_INTR_SLAVE(I2C_LED_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!I2C_LED_CY_SCBIP_V0) */

                    /* Clear address match and stop history */
                    I2C_LED_ClearSlaveInterruptSource(I2C_LED_INTR_SLAVE_ALL);

                    /* ACK the address byte */
                    I2C_LED_I2C_SLAVE_GENERATE_NACK;
                }
                else
            #endif /* (I2C_LED_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
                {
                    if(I2C_LED_CHECK_I2C_STATUS(I2C_LED_I2C_STATUS_S_READ))
                    /* TX direction: master reads from slave */
                    {
                        I2C_LED_SetTxInterruptMode(I2C_LED_INTR_TX_EMPTY);

                        /* Set temporary index to address buffer clear from API */
                        I2C_LED_slRdBufIndexTmp = I2C_LED_slRdBufIndex;

                        /* Start master reading */
                        I2C_LED_slStatus |= (uint8) I2C_LED_I2C_SSTAT_RD_BUSY;
                        I2C_LED_state     = I2C_LED_I2C_FSM_SL_RD;
                    }
                    else
                    /* RX direction: master writes into slave */
                    {
                        /* Calculate available buffer size */
                        diffCount = (I2C_LED_slWrBufSize - I2C_LED_slWrBufIndex);
                        ...
                        /* Receive data: into RX FIFO */
                        {
                            if(diffCount == I2C_LED_I2C_FIFO_SIZE)
                            {
                                ...
                                I2C_LED_ENABLE_SLAVE_AUTO_DATA;
                            }
                            else
                            {
                                /* Stretch clock when RX FIFO becomes FULL */
                                I2C_LED_ENABLE_SLAVE_AUTO_DATA_ACK;
                        ...
                        /* Start master reading */
                        I2C_LED_slStatus |= (uint8) I2C_LED_I2C_SSTAT_WR_BUSY;
                        I2C_LED_state     = I2C_LED_I2C_FSM_SL_WR;
                    }
                    ...
                    I2C_LED_ENABLE_INTR_SLAVE(I2C_LED_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!I2C_LED_CY_SCBIP_V0) */

                    /* ACK the address byte */
                    I2C_LED_I2C_SLAVE_GENERATE_ACK;
                }
            }
            ...
            */
            if(I2C_LED_CHECK_INTR_RX_MASKED(I2C_LED_INTR_RX_FULL))
            {
                ...
                diffCount =  I2C_LED_slWrBufSize -
                            (I2C_LED_slWrBufIndex + I2C_LED_I2C_FIFO_SIZE);

                if(diffCount > I2C_LED_I2C_FIFO_SIZE) /* Proceed transaction */
                {
                    diffCount   = I2C_LED_I2C_FIFO_SIZE;
                    endTransfer = 0u;  /* Continue active transfer */
                }
                ...
                {
                    I2C_LED_ENABLE_SLAVE_AUTO_DATA_NACK;
                ...
                I2C_LED_ClearRxInterruptSource(I2C_LED_INTR_RX_FULL);
            }
            ...
            */
            ...
            {
                diffCount = I2C_LED_RX_FIFO_RD_REG;

                if(I2C_LED_slWrBufIndex < I2C_LED_slWrBufSize)
                {
                    I2C_LED_I2C_SLAVE_GENERATE_ACK;

                    /* Put data into component buffer */
                    I2C_LED_slWrBufPtr[I2C_LED_slWrBufIndex] = (uint8) diffCount;
                    I2C_LED_slWrBufIndex++;
                }
            ...
            */
            if(I2C_LED_CHECK_INTR_TX_MASKED(I2C_LED_INTR_TX_EMPTY))
            {
                while(I2C_LED_I2C_FIFO_SIZE != I2C_LED_GET_TX_FIFO_ENTRIES)
                {
                    ...
                    */
                    if(I2C_LED_slRdBufIndexTmp < I2C_LED_slRdBufSize)
                    /* Data from buffer */
                    {
                        I2C_LED_TX_FIFO_WR_REG = (uint32) I2C_LED_slRdBufPtr[I2C_LED_slRdBufIndexTmp];
                        I2C_LED_slRdBufIndexTmp++;
                    }
                    else
                    /* Probably Overflow */
                    {
                        I2C_LED_TX_FIFO_WR_REG = I2C_LED_I2C_SLAVE_OVFL_RETURN;

                        if(I2C_LED_slOverFlowCount <= I2C_LED_I2C_TX_OVERFLOW_COUNT)
                        {
                            /* Get counter in range of overflow. */
                            I2C_LED_slOverFlowCount++;
                        }
                    }
                }
    ...
    */
    ...
    {
        I2C_LED_CTRL_REG &= (uint32) ~I2C_LED_CTRL_ENABLED; /* Disable scb IP */

        I2C_LED_state = I2C_LED_I2C_FSM_IDLE;

        I2C_LED_DISABLE_SLAVE_AUTO_DATA;
        I2C_LED_DISABLE_MASTER_AUTO_DATA;
        ...
        I2C_LED_ClearTxInterruptSource(I2C_LED_INTR_RX_ALL);
        I2C_LED_ClearRxInterruptSource(I2C_LED_INTR_TX_ALL);
        I2C_LED_ClearSlaveInterruptSource(I2C_LED_INTR_SLAVE_ALL);
        I2C_LED_ClearMasterInterruptSource(I2C_LED_INTR_MASTER_ALL);
    #endif /* (I2C_LED_CY_SCBIP_V0) */

        I2C_LED_CTRL_REG |= (uint32) I2C_LED_CTRL_ENABLED;  /* Enable scb IP */
    }

#ifdef I2C_LED_I2C_ISR_EXIT_CALLBACK
    I2C_LED_I2C_ISR_ExitCallback();
#endif /* I2C_LED_I2C_ISR_EXIT_CALLBACK */
