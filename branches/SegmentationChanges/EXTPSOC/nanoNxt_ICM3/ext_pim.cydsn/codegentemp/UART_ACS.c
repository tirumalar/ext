...
uint8 UART_ACS_initVar = 0u;
    ...
    volatile uint16 UART_ACS_rxBufferRead  = 0u;
    volatile uint16 UART_ACS_rxBufferWrite = 0u;
...
void  UART_ACS_WriteControlRegister(uint8 control) 
{
    ...
    *******************************************************************************/
    uint16 UART_ACS_GetByte(void) 
    {
        ...
        /* Protect variables that could change on interrupt */
        UART_ACS_DisableRxInt();
        locErrorStatus = (uint16)UART_ACS_errorStatus;
        UART_ACS_errorStatus = 0u;
        UART_ACS_EnableRxInt();
        return ( (uint16)(locErrorStatus << 8u) | UART_ACS_ReadRxData() );
    #else
        return ( ((uint16)UART_ACS_ReadRxStatus() << 8u) | UART_ACS_ReadRxData() );
    #endif /* UART_ACS_RX_INTERRUPT_ENABLED */
