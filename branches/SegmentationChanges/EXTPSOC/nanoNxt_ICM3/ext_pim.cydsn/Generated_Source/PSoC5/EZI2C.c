...
volatile uint8 EZI2C_curState;  /* Current state of I2C state machine */

/* Pointer to data exposed to I2C Master */
volatile uint8 * EZI2C_dataPtrS1;

/* Size of buffer1 in bytes */
volatile unsigned short EZI2C_bufSizeS1; 
...
volatile unsigned short EZI2C_wrProtectS1;
...
/* If two slave addresses, creat second set of varaibles  */
    ...
    volatile unsigned short EZI2C_bufSizeS2; 
    ...
    /* Offset where data is read only */
    volatile unsigned short EZI2C_wrProtectS2;
...
void EZI2C_Init(void) 
{
    ...
    */
    ...
    {   /* 100kHz or 400kHz - 16 samples/bit */
        EZI2C_CFG_REG &= ((uint8) ~EZI2C_CFG_CLK_RATE);
    }
       ...
       EZI2C_XCFG_REG |= EZI2C_XCFG_I2C_ON;
        ...
        /* Process sio_select and pselect */
            ...
            else    /* GPIO is used */
            {
                ...
                EZI2C_CFG_REG &= ((uint8) ~EZI2C_CFG_PSELECT);
            }
        #endif  /* (EZI2C_ADDRESSES == EZI2C_ONE_ADDRESS) */

    #endif /* (1u == EZI2C_ENABLE_WAKEUP) */
    ...
    EZI2C_dataPtrS1 = (volatile uint8 *)0u;
    EZI2C_rwOffsetS1 = 0u;
    EZI2C_rwIndexS1 = 0u;
    ...
    #if(EZI2C_ADDRESSES == EZI2C_TWO_ADDRESSES)
        EZI2C_dataPtrS2 = (volatile uint8 *)0u;
        EZI2C_rwOffsetS2 = 0u;
        EZI2C_rwIndexS2 = 0u;
...
void EZI2C_Enable(void) 
{
    ...
    (void) CyIntClearPending(EZI2C_ISR_NUMBER);
...
void EZI2C_Start(void) 
{
    if(0u == EZI2C_initVar)
    {
        EZI2C_Init(); /* Initialize component's parameters */
...
{
    /* Enable interrupt */
    (void) CyIntEnable(EZI2C_ISR_NUMBER);
}
...
{
    /* Disable interrupt */
    (void) CyIntDisable(EZI2C_ISR_NUMBER);
}
...
*
*******************************************************************************/
void EZI2C_SetBuffer1(unsigned short bufSize, unsigned short rwBoundry, volatile uint8 * dataPtr) 
{
    /* Check for proper buffer */
    if(NULL != dataPtr)
    {
        EZI2C_dataPtrS1   = dataPtr;  /* Set buffer pointer */
        EZI2C_bufSizeS1   = bufSize;
        EZI2C_wrProtectS1 = rwBoundry;
    }
    EZI2C_rwOffsetS1  = 0u;  /* Clears buffer offset */
    EZI2C_rwIndexS1   = 0u;  /* Clears buffer index */
}
    ...
    *
    *******************************************************************************/
    void EZI2C_SetBuffer2(unsigned short bufSize, unsigned short rwBoundry, volatile uint8 * dataPtr) 
    {
        /* Check for proper buffer */
        if(NULL != dataPtr)
        {
            EZI2C_dataPtrS2   = dataPtr;  /* Set buffer pointer */
            EZI2C_bufSizeS2   = bufSize;
            EZI2C_wrProtectS2 = rwBoundry;
        }
        EZI2C_rwOffsetS2  = 0u;  /* Clears buffer offset */
        EZI2C_rwIndexS2   = 0u;  /* Clears buffer index */
    }
