...
{
    #ifdef isr_tick_INTERRUPT_INTERRUPT_CALLBACK
        isr_tick_Interrupt_InterruptCallback();
    #endif /* isr_tick_INTERRUPT_INTERRUPT_CALLBACK */ 
...
{
    CyRamVectors[CYINT_IRQ_BASE + isr_tick__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + isr_tick__INTC_NUMBER];
}
...
*
*   Note calling isr_tick_Start or isr_tick_StartEx will 
*   override any effect this API would have had. This API should only be called
*   after isr_tick_Start or isr_tick_StartEx has been called. 
...
void isr_tick_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((isr_tick__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *isr_tick_INTC_PRIOR = (*isr_tick_INTC_PRIOR & (uint32)(~isr_tick__INTC_PRIOR_MASK)) |
...
uint8 isr_tick_GetPriority(void)
{
    ...
    priority = (*isr_tick_INTC_PRIOR & isr_tick__INTC_PRIOR_MASK) >> priorityOffset;
