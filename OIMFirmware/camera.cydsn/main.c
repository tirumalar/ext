...
#define SET_V 40000
#define MAX_DUTY 80
#define MAX_DUTY_V2 120
//95


#define HW_V1 1
#define HW_V2 2
uint8_t g_HWVersion=0;
...
void DoHWVersionDetect(void)
{
    IFB_SetDriveMode(IFB_DM_STRONG);
    IFB_Write(1);
    CyDelayUs(1);
    if (IFB_Read())
    {
        ...
        IFB_Write(0);
        IFB_SetDriveMode(IFB_DM_STRONG);
        MAX_I_SetDriveMode(MAX_I_DM_STRONG);
        MAX_I_Write(1);
        }
    else
        {
         g_HWVersion=HW_V2;
         IFB_Write(0);
         IFB_SetDriveMode(IFB_DM_ALG_HIZ);
        }
}
...
int main()
{
   ...
   DoHWVersionDetect();
    ...
    set.i_set=4;
    if (set.hwDetect=HW_V1)
        set.max_duty=MAX_DUTY;
    else
        set.max_duty=MAX_DUTY_V2;
...
//    OLD_SDA_Write(1);
   // Init_bb_i2c((uint8 *)&set.action);
    ...
    if (set.action==     PSOC_ACTION_REBOOT)
             CySoftwareReset();
