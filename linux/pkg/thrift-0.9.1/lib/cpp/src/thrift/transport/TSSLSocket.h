...
class TSSLSocket: public TSocket {
 ...
 ~TSSLSocket();
  ...
  /**
    * Determine if TLS 1.2 usage is forced
    *
    * @return true, if TLS 1.2 is forced
    */
  virtual bool forceTLS12() { return forceTLS12_; }
...
class EyelockTSSLSocket: public TSSLSocket {
 public:
 ~EyelockTSSLSocket();
protected:
  EyelockTSSLSocket(boost::shared_ptr<SSLContext> ctx);
  EyelockTSSLSocket(boost::shared_ptr<SSLContext> ctx, int socket);
  EyelockTSSLSocket(boost::shared_ptr<SSLContext> ctx,std::string host,int port);
  virtual void authorize();
  friend class TSSLSocketFactory;
};
...
class TSSLSocketFactory {
 public:
   ...
   */
  TSSLSocketFactory();
  TSSLSocketFactory(bool forceTLS12);
  ...
  virtual void setCertAuthenticationCallback(void *func,int mode);
  /**
  ...
  virtual void loadByteCertificates(const char* certificate);
  /**
   * Load private key.
   *
  ...
  virtual void loadBytePrivateKey(const char* Key );
  ...
  virtual void loadTrustedByteCertificates(const char* certificate);
  /**
 ...
 private:
  ...
  static unsigned long int count_;
  void setup(boost::shared_ptr<TSSLSocket> ssl);
  static int passwordCallback(char* password, int size, int, void* data);
  bool m_eyelockSocket;
  bool forceTLS12_;
};


class EyelockTSSLSocketFactory: public TSSLSocketFactory {
 public:
  EyelockTSSLSocketFactory();
  virtual ~EyelockTSSLSocketFactory();
  virtual boost::shared_ptr<TSSLSocket> createSocket();
  virtual boost::shared_ptr<TSSLSocket> createSocket(int socket);
  virtual boost::shared_ptr<TSSLSocket> createSocket(const std::string& host,int port);
private :
  boost::shared_ptr<AccessManager> access_;
  void setup(boost::shared_ptr<EyelockTSSLSocket> ssl);
