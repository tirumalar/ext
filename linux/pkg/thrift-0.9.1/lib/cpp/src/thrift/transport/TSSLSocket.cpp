...
unsigned long int TSSLSocketFactory::count_ = 0;
Mutex    TSSLSocketFactory::mutex_;

TSSLSocketFactory::TSSLSocketFactory(): server_(false),m_eyelockSocket(false),forceTLS12_(false) {
  Guard guard(mutex_);
  if (count_ == 0) {
    initializeOpenSSL();
    randomize();
...
TSSLSocketFactory::TSSLSocketFactory(bool forceTLS12): server_(false),m_eyelockSocket(false),forceTLS12_(forceTLS12) {
  Guard guard(mutex_);
  if (count_ == 0) {
    initializeOpenSSL();
    randomize();
  ...
  count_++;
  ctx_ = boost::shared_ptr<SSLContext>(new SSLContext(forceTLS12));
}
...
boost::shared_ptr<TSSLSocket> TSSLSocketFactory::createSocket() {
	boost::shared_ptr<TSSLSocket> ssl((TSSLSocket*) (m_eyelockSocket ? new EyelockTSSLSocket(ctx_):new TSSLSocket(ctx_)));
	setup(ssl);
    return ssl;
}

boost::shared_ptr<TSSLSocket> TSSLSocketFactory::createSocket(int socket) {
  boost::shared_ptr<TSSLSocket> ssl((TSSLSocket*) (m_eyelockSocket ? new EyelockTSSLSocket(ctx_,socket):new TSSLSocket(ctx_,socket)));
  setup(ssl);
  return ssl;
}

boost::shared_ptr<TSSLSocket> TSSLSocketFactory::createSocket(const string& host,int port) {
  boost::shared_ptr<TSSLSocket> ssl((TSSLSocket*) (m_eyelockSocket ? new EyelockTSSLSocket(ctx_,host,port):new TSSLSocket(ctx_,host,port)));
  setup(ssl);
  return ssl;
}
...
void TSSLSocketFactory::loadByteCertificates(const char* certificate)
{
	if (certificate == NULL) {
	    throw TTransportException(TTransportException::BAD_ARGS,
	         "loadTrustedByteCertificates: <certificate> is NULL");
	  }
	  if (SSL_CTX_use_certificate(ctx_->get(),(X509*)certificate) == 0) {

	    int errno_copy = THRIFT_GET_SOCKET_ERROR;
	    string errors;
	    buildErrors(errors, errno_copy);
	    throw TSSLException("SSL_use_certificate error: " + errors);
	  }
}
void TSSLSocketFactory::loadTrustedByteCertificates(const char* certificate) {
  if (certificate == NULL) {
    throw TTransportException(TTransportException::BAD_ARGS,
         "loadTrustedByteCertificates: <path> is NULL");
  }

  X509 *x509Cert =(X509*)certificate;
  X509_STORE *store = SSL_CTX_get_cert_store(ctx_->get());
  X509_STORE_add_cert(store, x509Cert);



  //if (SSL_CTX_add_client_CA(ctx_->get(), (X509*)certificate) == 0) {
    //int errno_copy = THRIFT_GET_SOCKET_ERROR;
   // string errors;
   // buildErrors(errors, errno_copy);
    //throw TSSLException("SSL_CTX_add_client_CA: " + errors);
  //}
}
...
void TSSLSocketFactory::loadBytePrivateKey(const char* Key )
{
	  if (Key == NULL ) {
	    throw TTransportException(TTransportException::BAD_ARGS,
	         "loadPrivateKey: is NULL");
	  }

	    if (SSL_CTX_use_PrivateKey(ctx_->get(), (EVP_PKEY*) Key) == 0)
	    {
	      int errno_copy = THRIFT_GET_SOCKET_ERROR;
	      string errors;
	      buildErrors(errors, errno_copy);
	      throw TSSLException("SSL_use_PrivateKey: " + errors);
	    }
}
