			...
			if (m_bFirstLaunch == false && m_dblLastCompletionFraction < m_dblCompletionFraction)
			{
				...
				if (width > 0 && height > 0)
				{
					...
					if (!pnlStatus.IsDisposed)
					{
						...
						LinearGradientBrush brBackground = new LinearGradientBrush(m_rProgress, Color.FromArgb(64, 198, 99), Color.FromArgb(64, 198, 99), LinearGradientMode.Horizontal);
