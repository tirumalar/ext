...
FPGA_VER=$(cat "${EYELOCK_WS_EXTLINUX}/OIMBinaries/versions.txt" | grep 'FPGA VERSION' | cut -d':' -f2)
FIXED_BRD_VER=$(cat "${EYELOCK_WS_EXTLINUX}/OIMBinaries/versions.txt" | grep 'Fixed board Verson' | cut -d':' -f2)
CAM_BRD_VER=$(cat "${EYELOCK_WS_EXTLINUX}/OIMBinaries/versions.txt" | grep 'Cam Psoc Version' | cut -d':' -f2)
...
cp "${EYELOCK_WS_EXTLINUX}/OIMBinaries/output12.bin" "${TARGET_DIR}/${FPGA_FILE}"
cp "${EYELOCK_WS_EXTLINUX}/OIMBinaries/fixed.cyacd" "${TARGET_DIR}/${FIXED_BRD_FILE}"
cp "${EYELOCK_WS_EXTLINUX}/OIMBinaries/camera.cyacd" "${TARGET_DIR}/${CAM_BRD_FILE}"

tar -cf "${EYELOCK_WS_EXTLINUX}/dist/${FW_FILE}" -C "${TARGET_DIR}" "${BOARD_FILE}" "${BOARD_FILE}.md5" "${ICM_FILE}" "${FPGA_FILE}" "${FIXED_BRD_FILE}" "${CAM_BRD_FILE}" "${FWHANDLER_FILE}" "${FWHANDLER_FILE}.sig" "${XML_FILE}" "${XML_FILE_LEGACY}"
