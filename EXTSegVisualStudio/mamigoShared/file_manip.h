...
#define END_TIMER gettimeofday(&_timer_2,0);printf("%35s:%llu\n",_timer_msg,tvdelta(&_timer_2,&_timer_1))
#define TIME_OP(m, op) { \
	DEF_TIMER(m); \
	START_TIMER; \
	op;	\
...
void savefile_OfSize_asPGM(unsigned char* image, short width, short height,const char *fileName,const char *comment);
