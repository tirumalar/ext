...
void clock_1k_Start(void) 
{
    ...
    clock_1k_CLKEN |= clock_1k_CLKEN_MASK;
	clock_1k_CLKSTBY |= clock_1k_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: clock_1k_Stop
********************************************************************************
*
* Summary:
...
void clock_1k_Stop(void) 
{
    ...
    clock_1k_CLKEN &= (uint8)(~clock_1k_CLKEN_MASK);
	clock_1k_CLKSTBY &= (uint8)(~clock_1k_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: clock_1k_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void clock_1k_StopBlock(void) 
{
    if ((clock_1k_CLKEN & clock_1k_CLKEN_MASK) != 0u)
    {
...
#if defined(clock_1k__CFG3)
        CLK_DIST_AMASK = clock_1k_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = clock_1k_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* clock_1k__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(clock_1k_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(clock_1k_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: clock_1k_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void clock_1k_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        clock_1k_CLKSTBY &= (uint8)(~clock_1k_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: clock_1k_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void clock_1k_SetDividerRegister(unsigned short clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = clock_1k_GetSourceRegister();
    unsigned short oldDivider = clock_1k_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = clock_1k_CLKEN & clock_1k_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((clock_1k_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: clock_1k_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
unsigned short clock_1k_GetDividerRegister(void) 
{
    return CY_GET_REG16(clock_1k_DIV_PTR);
}


/*******************************************************************************
* Function Name: clock_1k_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void clock_1k_SetModeRegister(uint8 modeBitMask) 
{
    clock_1k_MOD_SRC |= modeBitMask & (uint8)clock_1k_MODE_MASK;
}


/*******************************************************************************
* Function Name: clock_1k_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void clock_1k_ClearModeRegister(uint8 modeBitMask) 
{
    clock_1k_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(clock_1k_MODE_MASK));
}


/*******************************************************************************
* Function Name: clock_1k_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 clock_1k_GetModeRegister(void) 
{
    return clock_1k_MOD_SRC & (uint8)(clock_1k_MODE_MASK);
}


/*******************************************************************************
* Function Name: clock_1k_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void clock_1k_SetSourceRegister(uint8 clkSource) 
{
    unsigned short currDiv = clock_1k_GetDividerRegister();
    uint8 oldSrc = clock_1k_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        clock_1k_MOD_SRC |= CYCLK_SSS;
        clock_1k_MOD_SRC =
            (clock_1k_MOD_SRC & (uint8)(~clock_1k_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: clock_1k_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 clock_1k_GetSourceRegister(void) 
{
    return clock_1k_MOD_SRC & clock_1k_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: clock_1k_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void clock_1k_SetPhaseRegister(uint8 clkPhase) 
{
    clock_1k_PHASE = clkPhase & clock_1k_PHASE_MASK;
}


/*******************************************************************************
* Function Name: clock_1k_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 clock_1k_GetPhaseRegister(void) 
{
    return clock_1k_PHASE & clock_1k_PHASE_MASK;
}
