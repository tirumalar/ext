...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_UART_ACS) || \
                                          (CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Custom_Interface)))


/*******************************************************************************
* Function Name: UART_ACS_CyBtldrCommStart
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: UART_ACS_CyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: UART_ACS_CyBtldrCommReset
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: UART_ACS_CyBtldrCommWrite
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
cystatus UART_ACS_CyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut) CYSMALL
         
{
    ...
    while(bufIndex < size)
    {
        UART_ACS_PutChar(pData[bufIndex]);
        bufIndex++;
    }
...
/*******************************************************************************
* Function Name: UART_ACS_CyBtldrCommRead
********************************************************************************
*
* Summary:
...
*******************************************************************************/
cystatus UART_ACS_CyBtldrCommRead(uint8 pData[], uint16 size, uint16 * count, uint8 timeOut) CYSMALL
         
{
    ...
    *  If at least one byte is received within the time out interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        */
        if(0u != UART_ACS_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = UART_ACS_GetRxBufferSize();
                CyDelay(UART_ACS_BYTE2BYTE_TIME_OUT);
            }
            while(UART_ACS_GetRxBufferSize() > oldDataCount);

            status = CYRET_SUCCESS;
            break;
        }
        /* If the data is not received, give a delay of 
        *  UART_ACS_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(UART_ACS_BL_CHK_DELAY_MS);
        }
    }
    ...
    *count = 0u;
    ...
    while(UART_ACS_GetRxBufferSize() > 0u)
    {
        tempCount = UART_ACS_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = UART_ACS_ReadRxData();
                dataIndexCntr++;
            }

            /* Check if the last received byte is end of packet defined by bootloader
            *  If not wait for additional UART_ACS_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != UART_ACS_PACKET_EOP)
            {
                CyDelay(UART_ACS_WAIT_EOP_DELAY);
            }
        }
