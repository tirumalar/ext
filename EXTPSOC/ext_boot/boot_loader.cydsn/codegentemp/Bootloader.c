 ...
 */
static const uint8 CYCODE Bootloader_securityKey[Bootloader_SECURITY_KEY_LENGTH] = {0x11u, 0x22u, 0x33u, 0x44u, 0x55u, 0x66u};
...
#if (CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER)                                    
    static void Bootloader_HostLink(uint8 timeOut) CYLARGE ;
#endif /* (CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER) */                                    
#endif /*(CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LAUNCHER)*/
                                    
#if (((0u != Bootloader_DUAL_APP_BOOTLOADER) && (CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LAUNCHER)) \
                                          || (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER))
    static uint8 Bootloader_CheckImage(uint8 appNumber, uint8 arrayNumber, uint16 row, uint16 rowNumInArray) CYSMALL \
                                    ;                                    
#endif /*(((0u != Bootloader_DUAL_APP_BOOTLOADER) && (CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LAUNCHER)) \
                                          || (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER))*/

#if ((CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER) || (0u != Bootloader_DUAL_APP_BOOTLOADER)) 
    static void Bootloader_SetActiveAppInMetadata(uint8 activeApp) CYSMALL \
                ;
#endif /* (CYDEV_PROJ_TYPE == CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER) || (0u != Bootloader_DUAL_APP_BOOTLOADER) */                     
    ...
    ***************************************************************************/
    ...
    if (CYRET_SUCCESS == copyStatus)
    {
        ...
        ******************************************************************************/
        ...
        metaBuf[Bootloader_MD_BUFFER_START_OFFSET + Bootloader_MD_COPY_FLAG_BYTE_OFFSET] = Bootloader_COPIER_SUPPORT_SET_BIT;                
...
void Bootloader_Start(void) CYSMALL 
{
    ...
    ***********************************************************************/
           ...
           (0u == *Bootloader_SizeBytesAccess))
        {
            ...
            * the checksum stored in special Bootloader_Checksum constant 
            * (accessed through Bootloader_ChecksumAccess) constant or its size 
            * stored in Bootloader_SizeBytes constant (accessed through
            * Bootloader_SizeBytesAccess) then CPU is halted, all software 
            * processing stops. 
            ***********************************************************************/
            CyHalt(0x00u);
        }
...
*   - Bootloader_EXIT_TO_BTLDR - The Bootloader application will be started on
*                                     a software reset.
...
void CyBtldr_CheckLaunch(void) CYSMALL 
{
    ...
    *******************************************************************************/
    ...
    if (Bootloader_GET_RUN_TYPE == Bootloader_SCHEDULE_BTLDB)
    {
        ...
        * The active application number is taken from Bootloader_activeApp not
        * to break backward compatibility.
        *******************************************************************************/
        uint8 activeApp = Bootloader_activeApp;
        ...
        * between applications. So if there is no valid value for Bootloader_activeApp,
        ...
        *******************************************************************************/
        if ((0u != Bootloader_GetMetadata(Bootloader_GET_BTLDB_ADDR, activeApp)) 
...
cystatus Bootloader_ValidateBootloadable(uint8 appId) CYSMALL \

    {
        ...
        uint32 CYDATA end   = Bootloader_FIRST_APP_BYTE(appId) +
                              Bootloader_GetMetadata(Bootloader_GET_BTLDB_LENGTH, appId);
...
#endif /*(CYDEV_PROJ_TYPE != CYDEV_PROJ_TYPE_LOADABLEANDBOOTLOADER)*/    
{
    ...
    do
    {
        ...
        else
        {
            ...
            **************************************************************************************/
            if (((pktSize + Bootloader_MIN_PKT_SIZE) > numberRead) || 
                ((pktSize + Bootloader_MIN_PKT_SIZE) > Bootloader_SIZEOF_COMMAND_BUFFER)) 
            {
            ...
            else /* Packet length is OK*/
            {
                ...
                if (packetBuffer[Bootloader_EOP_ADDR(pktSize)] != Bootloader_EOP)
                {
                    ackCode = Bootloader_ERR_DATA;
                }
        ...
        if(ackCode == CYRET_SUCCESS)
        {
            ...
            ***************************************************************************/
        #if (0u != Bootloader_CMD_VERIFY_APP_CHKSUM_AVAIL)     
            case Bootloader_COMMAND_CHECKSUM:
                ...
                if((Bootloader_COMMUNICATION_STATE_ACTIVE == communicationState) && (pktSize == 0u))
                {
                    ...
                    rspSize = Bootloader_RSP_SIZE_VERIFY_CHKSM;
                    ackCode = CYRET_SUCCESS;
                }
                break;
        #endif /* (0u != Bootloader_CMD_VERIFY_APP_CHKSUM_AVAIL) */
                   ...
                   ((3u + Bootloader_FROW_SIZE) == pktSize)))
                {
                    ...
                    *******************************************************************************/                    
                        ...
                        if ((CYRET_SUCCESS == ackCode) && (btldrData < CY_FLASH_NUMBER_ARRAYS) && (dataOffset > rowSize))
                        {
                            ...
                            ackCode = Bootloader_VerifyRow(startAddr, &dataBuffer[rowSize], (uint16)CYDEV_ECC_ROW_SIZE);                        
                        }
                ...
                case Bootloader_COMMAND_REPORT_SIZE:
                    ...
                    if((Bootloader_COMMUNICATION_STATE_ACTIVE == communicationState) && (pktSize == 1u))
                    {
                        if(btldrData < CY_FLASH_NUMBER_ARRAYS)
                        {
                            ...
                            *   then we do not use the Bootloader_SizeBytes and Bootloader_SizeBytesAccess 
                            ...
                            *******************************************************************************/
                            ...
                            rspSize = Bootloader_RSP_SIZE_GET_FLASH_SIZE;
                            ackCode = CYRET_SUCCESS;
                        }
            ...
            case Bootloader_COMMAND_ERASE:
                ...
                if((Bootloader_COMMUNICATION_STATE_ACTIVE == communicationState) && (pktSize >= 3u))
                {
                    ...
                    if(dataOffset == pktSize)
                    {
                        ...
                        /*Checking if row number is within array address range*/
                        ackCode = Bootloader_CHECK_ROW_NUMBER(dataOffset, upperRange);
        ...
        #if (Bootloader_SECURITY_KEY_AVAIL == 1u)    
            ...
            ***************************************************************************/
            #if (0u != Bootloader_CMD_GET_ROW_CHKSUM_AVAIL)

            case Bootloader_COMMAND_GET_ROW_CHKSUM:
                ...
                if((Bootloader_COMMUNICATION_STATE_ACTIVE == communicationState) && (pktSize == 3u))
                {
                    ...
                    * correct.
                    ...
                    if((Bootloader_MD_FLASH_ARRAY_NUM == btldrData) && (Bootloader_CONTAIN_METADATA(tmpRowNum)))
                    {

                        checksum -= (uint8)Bootloader_GetMetadata(Bootloader_GET_BTLDB_ACTIVE,
                                                                 Bootloader_GET_APP_ID(tmpRowNum));
                    ...
                    ackCode = CYRET_SUCCESS;
                    rspSize = Bootloader_RSP_SIZE_VERIFY_ROW_CHKSM;
                }
                break;

            #endif /* (0u != Bootloader_CMD_GET_ROW_CHKSUM_AVAIL) */
            ...
            ***************************************************************************/
            ...
            #if (!CY_PSOC3)
                if ((NULL != Bootloader_callback) && (Bootloader_COMMUNICATION_STATE_ACTIVE == communicationState))
                {
...
uint32 Bootloader_GetMetadata(uint8 field, uint8 appId)
{
    ...
    case Bootloader_GET_BTLDB_COPY_FLAG:
        fieldPtr = Bootloader_MD_BTLDB_COPY_FLAG_OFFSET(appId);
        fieldSize = 1u;
        break;        

    case Bootloader_GET_BTLDB_USER_DATA:
        fieldPtr = Bootloader_MD_BTLDB_USER_DATA_OFFSET(appId);
        fieldSize = 4u;
        break;        
