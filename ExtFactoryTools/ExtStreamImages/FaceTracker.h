...
public:
	...
	void DoRunMode2Streams(bool bShowFaceTracking, bool bDebugSessions, int CameraType);
	cv::Point2i ProcessIrisImage2(IplImage *inputImage, cv::Mat faceImage, cv::Mat IrisImage, cv::Rect leftRect, cv::Rect rightRect);
	bool streamVideo2(int CameraType, cv::Rect FaceCoord, cv::Mat FaceImg);
	void validateLeftRightEyecrops2( int CameraIdLeft, int CameraIdRight, cv::Point2i ptrILeft, cv::Point2i ptrIRight, cv::Rect leftRect,
