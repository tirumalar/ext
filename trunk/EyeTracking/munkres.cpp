...
bool 
Munkres::find_uncovered_in_matrix(int item, int &row, int &col) {
	...
	for ( int row = 0 ; row < m_rows ; row++ )
		for ( int col = 0 ; col < m_columns ; col++ )
			if ( matrix[row][col] == 0 ) {
				...
				if ( !isstarred ) {
					for ( int ncol = 0 ; ncol < m_columns ; ncol++ )
...
Munkres::step2(void) {
	...
	int k = ((m_rows < m_columns) ? m_rows : m_columns);
...
void 
Munkres::solve(int **m, int rows, int cols) {
	...
	mask_matrix = new int*[m_rows];
	for( int i=0; i<m_rows; i++)
	{
		mask_matrix[i] = new int[m_columns];
		memset(mask_matrix[i], 0, sizeof(int)*m_columns);
	}

	row_mask = new bool[m_rows];
	col_mask = new bool[m_columns];
	for ( int i = 0 ; i < m_rows ; i++ ) {
		row_mask[i] = false;
	}
