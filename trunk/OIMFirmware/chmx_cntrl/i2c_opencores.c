...
alt_u32 mux_get_base(alt_u32 base)
    {
    alt_u32 mux_sel;
    static alt_u32 mux_sel_cur=-1;

    switch (base)
	{
	 case I2C_OPENCORES_0_BASE:mux_sel=0;break;
	 case I2C_OPENCORES_1_BASE:mux_sel=2;break;
	 case I2C_OPENCORES_2_BASE:mux_sel=1;break;
	 case I2C_OPENCORES_3_BASE:mux_sel=3;break;

	}
    if (mux_sel_cur!=mux_sel)
	{
        IOWR_ALTERA_AVALON_PIO_CLEAR_BITS(PIO_0_BASE,0x300);
        IOWR_ALTERA_AVALON_PIO_SET_BITS(PIO_0_BASE,mux_sel<<8);
	mux_sel_cur=mux_sel;
	}
    return I2C_OPENCORES_0_BASE;
    }
