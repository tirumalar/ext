...
copyFiles()
{
	...
	rm /home/eyelock/data/*.bin > /dev/null 2>&1
	sync
}
...
echo "no /home/root/eyestartup file"
	...
	if [ "$chkUserName" == "/home/eyelock" ] ; then
		...
		#usermod -p $(echo eyelock | openssl passwd -1 -stdin) eyelock
		usermod -p $(echo eyelock | openssl passwd -1 -stdin) -s /usr/sbin/nologin eyelock
