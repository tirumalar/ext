...
(function($){
	...
	bValidator = function(mainElement, overrideOptions, instanceName){
		...
		var options = {
			...
			errorMessages: {
				en: {
					...
					'pwcheck':		'Please enter a password which is at least 8 characters long,</br>uses at least one upper case, lower case letter, one digit and one symbol character.<br>Quotation marks not allowed'
				},
				us: {
					'default':    'Please correct this value.',
					'equalto':    'Please enter the same value again.',
				...
				pt: {
					'default':    'Por favor, digite um valor diferente.',
					'equalto':    'Por favor, digite o mesmo valor novamente.',
					'differs':    'Por favor, digite um valor diferente.',
					'minlength':  'O tamanho deve ser de pelo menos {0} caracteres',
					'maxlength':  'O tamanho máximo é de {0} caracteres',
					'rangelength':'O tamanho deve ter entre {0} e {1}',

					'min':        'Por favor, digite um número maior ou igual a {0}.',
					'max':        'Por favor, digite um número menor ou igual a {0}.',
					'between':    'Por favor, digite um número entre {0} e {1}.',
					'required':   'Este arquivo é obrigatório.',
					'alpha':      'Por favor, digite apenas caracteres alfabéticos.',
					'alphanum':   'Por favor, digite apenas caracteres alfanuméricos.',
					'digit':      'Por favor, digite somente números.',
					'number':     'Por favor, digite um número válido',
					'email':      'Por favor, digite ma endereço de email válido.',
					'image':      'Este campo deve conter somente imagens',
					'url':        'Por favor, digite uma url válida.',
					'ip4':        'Por favor, digite um endereço de IPv4 válido.',
					'ip6':        'Por favor, digite um endereço de IPv6 válido.',
					'date':       'Por favor, digite uma data válida no formato {0}.',
					'pwcheck':		'Por favor, digite uma senha com pelo menos 8 caracteres, usando </br> pelo menos uma letra minúscula, uma letra maiúscula, um número e um caracter especial. <br> Aspas não são permitidas' 
		...
		//DMOFIX	return element.is(':input') ? element : element.find(':input[' + options.validateActionsAttr + '], :input[' + options.modifyActionsAttr + ']').not(":button, :image, :reset, :submit, :hidden, :disabled");
        	return element.is(':input') ? element : element.find(':input[' + options.validateActionsAttr + '], :input[' + options.modifyActionsAttr + ']').not(":button, :image, :reset, :submit, :disabled");
		},
		...
		validator = {
			...
			pwcheck: function(password){
				if(password.value == 'installer' || password.value == 'admin') return true;
				return (password.value.length > 7)  &&
			// If password contains both lower and uppercase characters, increase strength value.
			 (password.value.match(/([a-z].*[A-Z])|([A-Z].*[a-z])/)) &&
			// If it has numbers and characters, increase strength value.
			 (password.value.match(/([a-zA-Z])/) && password.value.match(/([0-9])/))&&
			// If it has one special character, increase strength value.
			 (password.value.match(/([!,%,&,@,#,$,^,*,?,_,~])/))  // has a digit
			 &&
			  (!password.value.match(/([", '])/)); //no quotes allowed in password
			},
			min: function(v, min){		
			...
			subnetmask: function(v){
				return validator.regex(v, /^(((255\.){3}(255|254|252|248|240|224|192|128|0+))|((255\.){2}(255|254|252|248|240|224|192|128|0+)\.0)|((255\.)(255|254|252|248|240|224|192|128|0+)(\.0+){2})|((255|254|252|248|240|224|192|128|0+)(\.0+){3}))$/);
			},
