...
grep 169.254.1.1 /usr/etc/avahi/avahi-autoipd.action
if [ $? != 0 ]
then
        echo " updating the default ip"
        grep 169.254.1.2 /usr/etc/avahi/avahi-autoipd.action
        if [ $? != 1 ]
        then
                echo "configured with 169.254.1.2 address.Adjust it with .1"
                sed -i -e 's/169.254.1.2/169.254.1.1/g' /usr/etc/avahi/avahi-autoipd.action
        else
                echo "default ip is not configured at all"
                sed -i -e 's/"$3"/169.254.1.1/g' /usr/etc/avahi/avahi-autoipd.action
        fi
fi
...
then
	...
	ifconfig eth0 | grep -e "inet addr"
	if [ $? != 0 ]; then
		logger "failed to get dhcp ipadresses"
		/usr/etc/avahi/avahi-autoipd.action BIND eth0
	fi
fi
