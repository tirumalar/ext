...
uint8 Comp_ISENSE_initVar = 0u; /* Defines if component was initialized */
static uint32 Comp_ISENSE_internalSpeed = 0u; /* Defines component Speed value */


/*******************************************************************************
* Function Name: Comp_ISENSE_Init
********************************************************************************
*
* Summary:
...
void Comp_ISENSE_Init(void)
{
    ...
    Comp_ISENSE_internalSpeed = Comp_ISENSE_POWER;
    Comp_ISENSE_CTB_CTRL_REG = Comp_ISENSE_DEFAULT_CTB_CTRL;
    Comp_ISENSE_OA_RES_CTRL_REG = Comp_ISENSE_DEFAULT_OA_RES_CTRL;
    
    enableInterrupts = CyEnterCriticalSection();
    Comp_ISENSE_INTR_MASK_REG |= Comp_ISENSE_INTR_MASK_REG_DEFAULT;
    CyExitCriticalSection(enableInterrupts);
}


/*******************************************************************************
* Function Name: Comp_ISENSE_Enable
********************************************************************************
*
* Summary:
...
{
    Comp_ISENSE_OA_RES_CTRL_REG |= Comp_ISENSE_internalSpeed | \
                                        Comp_ISENSE_OA_PUMP_EN;
}


/*******************************************************************************
* Function Name: Comp_ISENSE_Start
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  Comp_ISENSE_initVar: Is modified when this function is called for the
*   first time. Is used to ensure that initialization happens only once.
*
*******************************************************************************/
void Comp_ISENSE_Start(void)
{
    if( 0u == Comp_ISENSE_initVar)
    {
        Comp_ISENSE_Init();
        Comp_ISENSE_initVar = 1u;
    }
    Comp_ISENSE_Enable();
}
...
/*******************************************************************************
* Function Name: Comp_ISENSE_Stop
********************************************************************************
*
* Summary:
...
{
    Comp_ISENSE_OA_CTRL_REG &= ((uint32)~(Comp_ISENSE_OA_PWR_MODE_MASK | \
                                               Comp_ISENSE_OA_PUMP_EN));
}


/*******************************************************************************
* Function Name: Comp_ISENSE_SetSpeed
********************************************************************************
*
* Summary:
...
*   Comp_ISENSE_SLOW_SPEED  - Slow speed
*   Comp_ISENSE_MED_SPEED   - Medium speed
*   Comp_ISENSE_FAST_SPEED  - Fast speed
*
* Return:
*  None
*
...
void Comp_ISENSE_SetSpeed(uint32 speed)
{
    ...
    Comp_ISENSE_internalSpeed = Comp_ISENSE_GET_OA_PWR_MODE(speed);
    tmp = Comp_ISENSE_OA_RES_CTRL_REG & \
           (uint32)~Comp_ISENSE_OA_PWR_MODE_MASK;
    Comp_ISENSE_OA_RES_CTRL_REG = tmp | Comp_ISENSE_internalSpeed;
}


/*******************************************************************************
* Function Name: Comp_ISENSE_ZeroCal
********************************************************************************
*
* Summary:
...
uint32 Comp_ISENSE_ZeroCal(void)
{
    ...
    Comp_ISENSE_OA_OFFSET_TRIM_REG       = 0u;
    Comp_ISENSE_OA_SLOPE_OFFSET_TRIM_REG = 0u;
    
    /* Check comparator's output state */
    cmpOut = Comp_ISENSE_GetCompare();
    ...
    */
    if(cmpOut == 0u)
    {
        Comp_ISENSE_OA_OFFSET_TRIM_REG       = ((uint32) 1u << Comp_ISENSE_COMP_TRIM_SIGN_SHIFT);
        Comp_ISENSE_OA_SLOPE_OFFSET_TRIM_REG = ((uint32) 1u << Comp_ISENSE_COMP_TRIM_SIGN_SHIFT);
    }
        
    /* Searching offset trim setting, 
    * that flips comparator output 
    */
    for(i = Comp_ISENSE_COMP_TRIM_MAX_VALUE; i != 0u; i--)
    {
        Comp_ISENSE_OA_OFFSET_TRIM_REG++;
        CyDelayUs(Comp_ISENSE_COMP_CALIBRATION_DELAY);
        if(cmpOut != Comp_ISENSE_GetCompare())
        {
            break;
        }
    }
    ...
    return (Comp_ISENSE_OA_OFFSET_TRIM_REG);
}


/*******************************************************************************
* Function Name: Comp_ISENSE_LoadTrim
********************************************************************************
*
* Summary:
...
{
    Comp_ISENSE_OA_OFFSET_TRIM_REG = trimVal;
}


/*******************************************************************************
* Function Name: Comp_ISENSE_GetCompare
********************************************************************************
*
* Summary:
...
{
    return ((uint32)(Comp_ISENSE_OA_COMP_STAT_REG >> Comp_ISENSE_OA_COMP_SHIFT) & 1u);
}


/*******************************************************************************
* Function Name: Comp_ISENSE_GetInterruptSource
********************************************************************************
*
* Summary:
...
{
    return (Comp_ISENSE_INTR_REG); 
}


/*******************************************************************************
* Function Name: Comp_ISENSE_ClearInterrupt
********************************************************************************
*
* Summary:
...
*  has a mask value: Comp_ISENSE_INTR.
*
* Return:
*  None
*
...
{   
    Comp_ISENSE_INTR_REG = interruptMask;
}


/*******************************************************************************
* Function Name: Comp_ISENSE_SetInterrupt
********************************************************************************
*
* Summary:
...
*  a mask value: Comp_ISENSE_INTR_SET.
*
* Return:
*  None
*
...
void Comp_ISENSE_SetInterrupt(uint32 interruptMask)
{
    ...
    enableInterrupts = CyEnterCriticalSection();
    Comp_ISENSE_INTR_SET_REG |= interruptMask;
    CyExitCriticalSection(enableInterrupts);
}


/*******************************************************************************
* Function Name: Comp_ISENSE_SetInterruptMask
********************************************************************************
*
* Summary:
...
*  component instance has a mask value: Comp_ISENSE_INTR_MASK.
*
* Return:
*  None
*
...
{
    Comp_ISENSE_INTR_MASK_REG = interruptMask;
}


/*******************************************************************************
* Function Name: Comp_ISENSE_GetInterruptMask
********************************************************************************
*
* Summary:
...
*  has a mask value: Comp_ISENSE_INTR_MASK.
*
*******************************************************************************/
uint32 Comp_ISENSE_GetInterruptMask(void)
{
    return (Comp_ISENSE_INTR_MASK_REG);
}


/*******************************************************************************
* Function Name: Comp_ISENSE_GetInterruptSourceMasked
********************************************************************************
*
* Summary:
...
*  has a mask value: Comp_ISENSE_INTR_MASKED.
*
*******************************************************************************/
uint32 Comp_ISENSE_GetInterruptSourceMasked(void)
{
    return (Comp_ISENSE_INTR_MASKED_REG);
}

/*******************************************************************************
* Function Name: Comp_ISENSE_SetInterruptMode
********************************************************************************
*
* Summary:
...
*  Comp_ISENSE_INTR_DISABLE - Disable
*  Comp_ISENSE_INTR_RISING  - Rising edge detect
*  Comp_ISENSE_INTR_FALLING - Falling edge detect
*  Comp_ISENSE_INTR_BOTH    - Detect both edges
*
* Return:
*  None
*
...
void Comp_ISENSE_SetInterruptMode(uint32 mode)
{
    ...
    tmp = Comp_ISENSE_OA_RES_CTRL_REG & (uint32)~Comp_ISENSE_OA_COMPINT_MASK;
    Comp_ISENSE_OA_RES_CTRL_REG = tmp | Comp_ISENSE_GET_OA_COMPINT(mode);
}

/*******************************************************************************
* Function Name: Comp_ISENSE_DisableInterruptOutput
********************************************************************************
*
* Summary:
...
void Comp_ISENSE_DisableInterruptOutput(void)
{
    ...
    enableInterrupts = CyEnterCriticalSection();
    Comp_ISENSE_INTR_MASK_REG &= ~Comp_ISENSE_INTR_MASK;
    CyExitCriticalSection(enableInterrupts);
}
