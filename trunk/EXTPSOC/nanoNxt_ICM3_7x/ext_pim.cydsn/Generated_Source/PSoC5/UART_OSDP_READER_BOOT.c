...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_UART_READER) || \
                                          (CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Custom_Interface)))


/*******************************************************************************
* Function Name: UART_READER_CyBtldrCommStart
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: UART_READER_CyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: UART_READER_CyBtldrCommReset
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: UART_READER_CyBtldrCommWrite
********************************************************************************
*
* Summary:
         ...
         
{
    ...
    while(bufIndex < size)
    {
        UART_READER_PutChar(pData[bufIndex]);
        bufIndex++;
    }
...
/*******************************************************************************
* Function Name: UART_READER_CyBtldrCommRead
********************************************************************************
*
* Summary:
         ...
         
{
    ...
    unsigned short dataIndexCntr;
    unsigned short tempCount;
    unsigned short oldDataCount;
        ...
        */
        if(0u != UART_READER_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = UART_READER_GetRxBufferSize();
                CyDelay(UART_READER_BYTE2BYTE_TIME_OUT);
            }
            while(UART_READER_GetRxBufferSize() > oldDataCount);

            status = CYRET_SUCCESS;
            break;
        }
        /* If the data is not received, give a delay of 
        *  UART_READER_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(UART_READER_BL_CHK_DELAY_MS);
        }
    }
    ...
    *count = 0u;
    ...
    while(UART_READER_GetRxBufferSize() > 0u)
    {
        tempCount = UART_READER_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = UART_READER_ReadRxData();
                dataIndexCntr++;
            }

            /* Check if the last received byte is end of packet defined by bootloader
            *  If not wait for additional UART_READER_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != UART_READER_PACKET_EOP)
            {
                CyDelay(UART_READER_WAIT_EOP_DELAY);
            }
        }
