...
/*******************************************************************************
* Function Name: clock_1kHz_Start
********************************************************************************
*
* Summary:
...
void clock_1kHz_Start(void) 
{
    ...
    clock_1kHz_CLKEN |= clock_1kHz_CLKEN_MASK;
	clock_1kHz_CLKSTBY |= clock_1kHz_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: clock_1kHz_Stop
********************************************************************************
*
* Summary:
...
void clock_1kHz_Stop(void) 
{
    ...
    clock_1kHz_CLKEN &= (uint8)(~clock_1kHz_CLKEN_MASK);
	clock_1kHz_CLKSTBY &= (uint8)(~clock_1kHz_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: clock_1kHz_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((clock_1kHz_CLKEN & clock_1kHz_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = clock_1kHz_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = clock_1kHz_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* clock_1kHz__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(clock_1kHz_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(clock_1kHz_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: clock_1kHz_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: clock_1kHz_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = clock_1kHz_GetSourceRegister();
    unsigned short oldDivider = clock_1kHz_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = clock_1kHz_CLKEN & clock_1kHz_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((clock_1kHz_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: clock_1kHz_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(clock_1kHz_DIV_PTR);
}


/*******************************************************************************
* Function Name: clock_1kHz_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    clock_1kHz_MOD_SRC |= modeBitMask & (uint8)clock_1kHz_MODE_MASK;
}


/*******************************************************************************
* Function Name: clock_1kHz_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    clock_1kHz_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(clock_1kHz_MODE_MASK));
}


/*******************************************************************************
* Function Name: clock_1kHz_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return clock_1kHz_MOD_SRC & (uint8)(clock_1kHz_MODE_MASK);
}


/*******************************************************************************
* Function Name: clock_1kHz_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    unsigned short currDiv = clock_1kHz_GetDividerRegister();
    uint8 oldSrc = clock_1kHz_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        clock_1kHz_MOD_SRC |= CYCLK_SSS;
        clock_1kHz_MOD_SRC =
            (clock_1kHz_MOD_SRC & (uint8)(~clock_1kHz_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: clock_1kHz_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return clock_1kHz_MOD_SRC & clock_1kHz_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: clock_1kHz_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    clock_1kHz_PHASE = clkPhase & clock_1kHz_PHASE_MASK;
}


/*******************************************************************************
* Function Name: clock_1kHz_GetPhase
********************************************************************************
*
* Summary:
...
{
    return clock_1kHz_PHASE & clock_1kHz_PHASE_MASK;
}
