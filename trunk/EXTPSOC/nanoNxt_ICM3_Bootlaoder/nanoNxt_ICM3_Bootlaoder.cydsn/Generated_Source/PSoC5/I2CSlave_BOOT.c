...
static uint8 XDATA I2CSlave_slReadBuf[I2CSlave_BTLDR_SIZEOF_READ_BUFFER];

/* I2C read buffer: Host reads responses from it */
static uint8 XDATA I2CSlave_slWriteBuf[I2CSlave_BTLDR_SIZEOF_WRITE_BUFFER];


/*******************************************************************************
* Function Name: I2CSlave_CyBtldrCommStart
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  I2CSlave_slWriteBuf - used to store received command.
*  I2CSlave_slReadBuf - used to store response.
...
{
    /* Set Write buffer */
    I2CSlave_SlaveInitWriteBuf(I2CSlave_slWriteBuf, I2CSlave_BTLDR_SIZEOF_WRITE_BUFFER);

    /* Set Read buffer which has zero elements */
    I2CSlave_SlaveInitReadBuf(I2CSlave_slReadBuf, 0u);
...
/*******************************************************************************
* Function Name: I2CSlave_CyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: I2CSlave_CyBtldrCommReset
********************************************************************************
*
* Summary:
...
void I2CSlave_CyBtldrCommReset(void) CYSMALL 
{
    ...
    I2CSlave_slStatus = 0u;
}


/*******************************************************************************
* Function Name: I2CSlave_CyBtldrCommWrite
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
cystatus I2CSlave_CyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut) CYSMALL
...
/*******************************************************************************
* Function Name: I2CSlave_CyBtldrCommRead
********************************************************************************
*
* Summary:
         ...
         
{
    ...
    if((NULL != pData) && (size > 0u))
    {
        ...
        while(0u != timeoutMs)  /* Wait for command from the host */
        {
            ...
            if(0u != (I2CSlave_slStatus & I2CSlave_SSTAT_WR_CMPLT))
            {
                /* How many bytes the host has been written */
                byteCount = I2CSlave_slWrBufIndex;
                *count = (uint16) byteCount;

                /* Copy command to pData buffer */
                (void) memcpy((void *) pData, (void *) I2CSlave_slWriteBuf,
                              I2CSlave_MIN_UNT16(byteCount, size));
