...
using ImageLogViewer.UI.Settings;
...
namespace ImageLogViewer
{
	public partial class MainWindow : GradientBaseForm
	{
		...
		private void SendMessageToDevice(string theMessage)
		{
			...
			try
			{
				client = new TcpClient(Properties.Settings.Default.DeviceIPAddress, Int32.Parse(Properties.Settings.Default.DevicePort));
		...
		private void MainWindow_Load(object sender, EventArgs e)
		{
			...
			// Force all tab pages into existence...
			TabControl.TabPageCollection thePages = tabControl1.TabPages;

			foreach (TabPage thePage in thePages)
			{
				thePage.Show();
			}
			...
			// Start listening on image port
			RefreshImageListening();

			// Start all listening on text file...
			remoteLogControlNXTLog.RefreshListening();
			remoteLogFaceTrackingLog.RefreshListening();
			remoteLogMotorLog.RefreshListening();
			remoteLogEventLog.RefreshListening();
		...
		private void RefreshImageListening()
		{ 
			int nPort;
		...
		{
			DeviceSettings dlgSettings = new DeviceSettings();

			dlgSettings.Owner = this;
			dlgSettings.StartPosition = FormStartPosition.CenterParent;
